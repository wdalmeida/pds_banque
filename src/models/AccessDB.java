...
public class AccessDB implements Constants {
...
/*
    ...
    */
    ...
    public ArrayList<String> getStatus() throws SQLException {
     ...
     public ArrayList getAllLoansOfCustomer(int idCustomer) throws SQLException {
        String query = "test";
        int i = 0;
        try {
            ArrayList tab = new ArrayList();
            query = "SELECT description_LoanRef,capital_Sim,percentage_Rate,monthly_Sim,amount_Insurance,duration_Sim FROM LoanSimulation Natural Join LoanRef Natural Join Rate Natural Join Insurance";
            ResultSet rs = this.declaration.executeQuery(query);
            if (rs.first()) {
                do {
                    tab.add(rs.getString(2));
